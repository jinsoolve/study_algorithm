## [백준 1168 - 요세푸스 문제 2](https://www.acmicpc.net/problem/1168)

### 풀이
세그먼트 트리를 이용해서 풀었다.
1. 현재 인덱스 k라 하자. (입력으로 N, K가 주어졌다. <span style="color:grey">k와 K는 다른 값이다.</span>)
2. k 이전 번호(1~k-1) 중에서 현재 남아있는 사람의 수가 s명이라 하자.
3. 이미 s명은 지나있으므로 다음 k는 s+K번째 사람이 될 것이다.
4. 원이기 때문에 (s+K) % (남아있는 전체 사람 수) 를 해준다.   
