## [백준 11495 - 격자 0 만들기](https://www.acmicpc.net/problem/11495)
...
### 풀이
1. 격자를 체스판으로 만든다. (흰칸 -> 검은칸, 검은칸 -> 흰칸이 될 것이다.)
2. 각 칸을 정점분할한다.
3. src -> 모든 흰칸, 모든 검은칸 -> sink으로 만든다.
4. src -> sink 로의 최대 유량값 maxFlow를 계산한다.
5. maxFlow는 두 칸을 골랐을 때 두 정수 모두 양수일 때 연산을 반복한 횟수를 말한다.  
   한 칸만 양수인 값은 (격자값의 모든 합을 sum이라 할 때) sum - 2*maxFlow 이 된다. 
6. 정답은 maxFlow + (sum - 2*maxFlow) = sum - maxFlow가 된다.
